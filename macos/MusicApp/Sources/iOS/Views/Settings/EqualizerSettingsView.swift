...
struct EQCurveView: View {
        ...
        VStack(spacing: 0) {
            ZStack {
                GeometryReader { geo in
                    ...
                    if enabled {
                        let curve = responseCurve(width: w, height: h)
                        let zeroY = dbToY(0, height: h)

                        // Fill
                        fillFromCurve(curve, zeroY: zeroY, width: w)
                            .fill(
                                LinearGradient(
    ...
    private func fillFromCurve(_ curve: Path, zeroY: Double, width: Double) -> Path {
        var path = curve
